...
**A. Summary measures for one variable**
...
**B. Summary measures for more variables**
...
We can also calculate the mean if the variable is numeric with the `summarize_if()` function:
```{r}
...
We are not limited to grouping by one variable. Say we want to know the mean QRS of males and females in each of the four bmi catecories:
...
Let’s look at some summary statistics of the `QRS` variable by considering multiple statistical functions at once in the same `summarize()` code:
...
```{r}
weight_summary <- arrhythmia_adults %>% 
  summarize(
...
The `age` variable approximate a **symmetrical distribution (bell-shaped)**.
...
### Frequency distribution table with percentages inside for each category

However, we may want to calculate the percentage inside for each category. For example, the percentage of males (n = 4) in the underweigt category (n = 11) is 36.4\%, (4/11)\*100. We will use the functions `group_by()`, `summarize()` and `mutate()` from the `dplyr` package:
