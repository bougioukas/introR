...
1. It’s a platform-independent language. This means it can be applied to all operating systems.
1. It’s an open-source free language. That means anyone can install it in any organization without purchasing a license.
1. R programming language is not only a statistic software but also allows us to integrate with other languages (C, C++). Thus, we can easily interact with many data sources and statistical tools.
1. The R programming language has a vast community of users and it’s growing day by day.
1. R is currently one of the most requested programming languages in the Data Science job market that makes it the hottest trend nowadays.
