...
First, let's take a look at the structure of the object _patient_data_ with the `glimpse()`:
...
The type is a _list_ but the class is a `tbl` _(tibble)_ object which is a "tidy" data frame. The dimensions are 8x6.
...
To access the variable _age_, we can use the **dollar sign (`$`)** like this:
